...
class PosListCategories extends Module
{
	...
	public function install()
	{
		...
		foreach ($shops as $shop_id)
		{
			...
			$res &= Configuration::updateValue($this->name.'_nsub', $this->default_nsub, false, $shop_group_id, $shop_id);
		}
		...
		if (count($shop_groups_list))
		{
			foreach ($shop_groups_list as $shop_group_id)
			{
				...
				$res &= Configuration::updateValue($this->name.'_nsub', $this->default_nsub, false, $shop_group_id);
			}
		}
		...
		$res &= Configuration::updateValue($this->name.'_nsub', $this->default_nsub);
			/* Creates tables */
	...
	public function uninstall()
	{
		...
		if (parent::uninstall())
		{
			...
			$res &= Configuration::deleteByName($this->name.'_nsub');
	...
	protected function _postProcess()
	{
		...
		if (Tools::isSubmit('submitPosProductCates'))
		{
			...
			foreach ($shops as $shop_id)
			{
				...
				$res &= Configuration::updateValue($this->name.'_nsub', (int)Tools::getValue($this->name.'_nsub'), false, $shop_group_id, $shop_id);
			}
			...
			switch ($shop_context)
			{
				case Shop::CONTEXT_ALL:
					...
					$res &= Configuration::updateValue($this->name.'_nsub', (int)Tools::getValue($this->name.'_nsub'));
					if (count($shop_groups_list))
					{
						foreach ($shop_groups_list as $shop_group_id)
						{
							...
							$res &= Configuration::updateValue($this->name.'_nsub', (int)Tools::getValue($this->name.'_nsub'), false, $shop_group_id);
						}
					}
					break;
	...
	public function hookblockPosition3($params)
	{
		...
		$number_sub = $config[$this->name.'_nsub'];
		$this->smarty->assign('number_sub', $number_sub);
		 ...
		 $configure = array(
			...
			if (is_array($items))
				...
				{	
					...
					$list_subcategories= Category::getChildren($category->id_category,$id_lang);
					 foreach($list_subcategories as $subcategory){
					 	$category =  new Category((int) $subcategory['id_category'], (int) $id_lang, (int) $id_shop);
						$subcategories[]= array(
							'name' => $category->name,
			...
			'form' => array(
				...
				'input' => array(
					...
					array(
						...
						'name' => 'poslistcategories_nsub',
						'class' => 'fixed-width-sm',
						'desc' => $this->l('How many subcategories do you want to show ?. Default is 3.')
					),
				),
				'submit' => array(
					'title' => $this->l('Save'),
	...
	public function getConfigFieldsValues()
	{
		...
		return array(
		   ...
		   $this->name.'_nsub' => Tools::getValue($this->name.'_nsub', Configuration::get($this->name.'_nsub', null, $id_shop_group, $id_shop))
		);
	}
