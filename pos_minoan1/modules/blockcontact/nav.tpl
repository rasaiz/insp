...
{if $telnumber}
	<div class="shop-phone{if isset($is_logged) && $is_logged} is_logged{/if}">
		<span><i class="icon-phone"></i>{l s='Phone:' mod='blockcontact'} </span><span><strong>{$telnumber}</strong></span>
	</div>
{/if}
{if $email != ''}
	<span class="shop-email">{l s='Email:' mod='blockcontact'}<a href="mailto:{$email|escape:'html':'UTF-8'}"> {$email}</a></span>
{/if}
</div>
